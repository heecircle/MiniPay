...
public class AccountService {
    ...
    public Long createAccount(CreateAccountRequest createAccountRequest){
        ...
        Account account = Account.builder().name(createAccountRequest.getName()).isMain(false).password(createAccountRequest.getPassword()).interestRate(createAccountRequest.getInterestRate()).user(user).build();
    ...
    public Long withdrawAccount2Account(WithdrawRequest withdrawRequest) throws BalanceException {
        Account withdrawAccount = accountRepository.findById(withdrawRequest.getAccountId()).orElseThrow(() -> new NotFoundException(""));
        Account insertAccount = accountRepository.findById(withdrawRequest.getPutAccountId()).orElseThrow(() -> new NotFoundException(""));
        User user = userRepository.findById(withdrawRequest.getPutAccountId()).orElseThrow(() -> new NotFoundException(""));
